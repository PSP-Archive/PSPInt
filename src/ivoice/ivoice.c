...
//#define DEBUG_FIFO
#ifdef DEBUG_FIFO
#define dfprintf(x) printf x ; fflush(stdout)
#else
#define dfprintf(x)
#endif
...
static const char rcs_id[]="$Id: ivoice.c,v 1.10 2001/11/02 02:00:03 im14u2c Exp $";
...
LOCAL uint_8 wav_hdr[44] =
{
    0x52, 0x49, 0x46, 0x46, 0x28, 0xC8, 0x00, 0x00, 
    0x57, 0x41, 0x56, 0x45, 0x66, 0x6D, 0x74, 0x20,
    0x10, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 
    0x10, 0x27, 0x00, 0x00, 0x20, 0x4E, 0x00, 0x00,
    0x02, 0x00, 0x10, 0x00, 0x64, 0x61, 0x74, 0x61, 
    0x00, 0x00, 0x00, 0x00  /* these four bytes are WAV file length */
};
...
LOCAL void iv_smp_uphdr(ivoice_t *iv)
{
    ...
    tot_bytes = iv->smp_totsmp * 2;

    fseek(iv->smp_file, 40, SEEK_SET);
    fputc( tot_bytes       & 0xFF, iv->smp_file);
    fputc((tot_bytes >>  8)& 0xFF, iv->smp_file);
    fputc((tot_bytes >> 16)& 0xFF, iv->smp_file);
    fputc((tot_bytes >> 24)& 0xFF, iv->smp_file);
...
LOCAL void iv_smp_start(ivoice_t *iv)
{
    ...
    {
        iv->smp_number++;
        iv->smp_cksum = realloc(iv->smp_cksum, 
...
printf("starting sample: %s\n", iv->smp_cname); fflush(stdout);
    }
}
...
printf("stopping sample: %s  checksum: %.8X  samples: %d\n", iv->smp_cname, iv->smp_cursum, iv->smp_totsmp);
    ...
    if (i != iv->smp_number)
    {
printf("sample was a repeat of earlier sample: %d\n", i);
        unlink(iv->smp_cname);
        return;
    }


    iv->smp_cksum = realloc(iv->smp_cksum, sizeof(uint_32)*(iv->smp_number+1));
...
LOCAL int stage_map[6] = { 0, 1, 2, 3, 4, 5 };
    ...
    while (ivoice->sound_current < until)
    {
        ...
        while (ivoice->sc_tail < ivoice->sc_head)
        {
            ...
            while (ivoice->sample_frc > 3579545)
            {
                ...
                /* -------------------------------------------------------- */
                if (ivoice->cur_len >= snd_buf_size)
                {
                    /* ---------------------------------------------------- */
        ...
        if (samples > 0) do 
        {
            ...
            if (ivoice->silent && ivoice->filt.rpt <= 0)
            {
        ...
        } while (ivoice->filt.rpt >= 0 && samples > did_samp);
        if (did_samp)
...
(
    ...
    char            *smp_tname  /*  Filename template for saving samples.   */
)
{
    int i;
    ...
    if (smp_tname)
    {
        ...
        {
            if ((ivoice->smp_cname = calloc(1, strlen(smp_tname)+10)) == NULL)
            {
