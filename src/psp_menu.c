...
psp_menu_display_save_name()
{
  ...
  snprintf(buffer, 20, "Game: %s", INTEL.intel_save_name);
...
psp_display_screen_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_ITEM; menu_id++) {
    ...
    if (menu_id == MENU_SCREENSHOT) {
      sprintf(buffer,"%d", INTEL.psp_screenshot_id);
...
# if 0
  ...
  for (slot_id = 0; slot_id < INTEL_MAX_SAVE_STATE; slot_id++) {
    if (slot_id == cur_slot) color = PSP_MENU_SEL2_COLOR;
    else                     color = PSP_MENU_TEXT_COLOR;

    if (INTEL.intel_save_used[slot_id]) sprintf(buffer, "%d - used" , slot_id + 1);
    else                          sprintf(buffer, "%d - empty", slot_id + 1);
    string_fill_with_space(buffer, 10);
    psp_sdl_back2_print(150, y, buffer, color);
...
psp_main_menu_reset(void)
{
  ...
  psp_sdl_back2_print(190, 160, "RESET !", 
...
psp_main_menu_load(int format)
{
  ...
  if (ret ==  1) /* load OK */
  {
    psp_menu_dirty = 1;
    psp_display_screen_menu();
    psp_sdl_back2_print(170, 160, "File loaded !", 
                       PSP_MENU_NOTE_COLOR);
    psp_menu_dirty = 1;
    psp_sdl_flip();
    sleep(1);
  ...
  if (ret == -1) /* Load Error */
  {
    psp_menu_dirty = 1;
    psp_display_screen_menu();
    psp_sdl_back2_print(170, 160, "Can't load file !", 
                       PSP_MENU_WARNING_COLOR);
    psp_menu_dirty = 1;
    psp_sdl_flip();
    sleep(1);
...
psp_main_menu_load_slot()
{
  ...
  if (! INTEL.intel_save_used[cur_slot]) {
  ...
  error = intel_snapshot_load_slot(cur_slot);
...
{
  int error;
  error = intel_snapshot_save_slot(cur_slot);
...
psp_main_menu_del_slot()
{
  ...
  error = intel_snapshot_del_slot(cur_slot);
...
{
  if (step == 1) {
    cur_slot++; if (cur_slot >= INTEL_MAX_SAVE_STATE) cur_slot = 0;
  } else if (step == -1) {
    cur_slot--; if (cur_slot < 0) cur_slot = INTEL_MAX_SAVE_STATE - 1;
  }
}
# endif
