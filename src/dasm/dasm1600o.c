...
int main(int argc, char **argv)
{
    ...
    for(addr=org;addr<endaddr;addr++)
    {
        ...
        if (datatype[addr] & 1)
        {
            ...
            sprintf(display_buffer.opcode_field,".word");
            sprintf(display_buffer.operand1_field,"$%04hx",w1);
            s = 1;
        }
        else
        {
    ...
    if (pass == 2)
    {
        ...
        if (datatype[addr] & 2)
            sprintf(display_buffer.label_field,"L%04hx:",addr);
