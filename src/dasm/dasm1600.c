...
#ifdef macintosh
/* Prototypes required for Macintosh */
...
int main(int argc, char **argv)
{
    ...
    while(fgets(buffer, 256, fpsym) && !done)
    {
        int args; 
        found = 0;
        args = sscanf(buffer,"%s %x %x", command, &temp, &temp2);
        i = 0;
        while((i<6) && !found)
        {
    ...
    for(addr=org;addr<endaddr;addr++)
    {
        ...
        sprintf(display_buffer.address_field,"%.4X",addr);
            ...
            (datatype[addr] & 16) && (datatype[addr+1] & 16))
        {
            ...
            w2 = code[addr+1];
            dbd = 1;
            sprintf(display_buffer.word1_field,"%.4X",w1);
            sprintf(display_buffer.word2_field,"%.4X",w2);
        ...
        } else
        {
            ...
            switch (s)
            {
                ...
                case 3:
                {
                    ...
                    sprintf(display_buffer.word1_field,"%.4X",w1);
                    sprintf(display_buffer.word2_field,"%.4X",w2);
                    sprintf(display_buffer.word3_field,"%.4X",w3);
                    display_buffer.char_field[1] = (w1>=32 && w1<=126) ? w1 : '.';
                    display_buffer.char_field[2] = (w2>=32 && w2<=126) ? w2 : '.';
