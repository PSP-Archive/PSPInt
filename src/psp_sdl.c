...
{
  if (! splash_surface) {
    splash_surface = IMG_Load("./splash.jpg");
  }
...
psp_sdl_save_screenshot(void)
{
  ...
  sprintf(TmpFileName,"%s/scr/screenshot_%d.png", INTEL.intel_home_dir, INTEL.psp_screenshot_id++);
  if (INTEL.psp_screenshot_id >= 10) INTEL.psp_screenshot_id = 0;
