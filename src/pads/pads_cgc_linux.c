...
/* ======================================================================== */
int pad_cgc_scanner(void *opaque)
{
...
//      if (write(pad->fd, &o_byte[1], 1) != 1) goto error;
        if (write(pad->fd, &o_byte[0], 2) != 2) goto error;
        if (tcflush(pad->fd, TCOFLUSH))         goto error;
        if (read (pad->fd, &i_byte[0], 1) != 1) goto error;
        if (read (pad->fd, &i_byte[1], 1) != 1) goto error;
        if (tcflush(pad->fd, TCIFLUSH))         goto error;
...
(
    ...
    char            *cgc_dev        /*  path to CGC device.                 */
)
{
    static int reaper = 0;
    SDL_Thread *th;
    ...
    */
    ...
    /* -------------------------------------------------------------------- */
    /*  Arrange to reap scanning threads when emulator exits.               */
    /* -------------------------------------------------------------------- */
    if (!reaper)
    {
        atexit(pad_cgc_reaper);
        reaper = 1;
    }
    ...
    printf("pads_cgc:  CGC @ \"%s\" (fd %d) mapped to $%.4X-$%.4X\n", 
